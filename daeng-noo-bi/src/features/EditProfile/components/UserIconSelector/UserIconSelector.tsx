...
const UserIconSelector = () => {
  ...
  const currentSrc = selectedIcon ? icons[selectedIcon - 1] : "/userIcon.png";
  ...
  return (
    <div className="UserIconSelector" ref={wrapperRef} aria-label="아이콘 변경">
      <label>아이콘 변경</label>
      <button
      ...
      >
        {/* 현재 아이콘 */}
        <img src={currentSrc} alt="현재 아이콘" className="selected-icon" />
      </button>

      {/* 아이콘 선택창 */}
      {isOpen && (
        <div className="icon-list" aria-label="아이콘 선택창">
          <Swiper
            modules={[FreeMode]}
            freeMode={true}
            spaceBetween={1}
